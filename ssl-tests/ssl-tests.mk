...
SSLTESTS_WIRESHARK_CAPTURE_PARAM := $(shell if [ 1 = "$(SSLTESTS_WIRESHARK_CAPTURE)" ] ; then  printf '%s' '-Dssltests.wiresharkCapture=1' ; fi )
...
ssl-tests-run: $(SSLTESTS_CLASSES_DIR) $(JAVA_SECURITY_DEPS) $(SSLTESTS_NSSDB_DEP)
	...
	$(SSLTESTS_ONLY_SSL_DEFAULTS_PARAM) $(SSLTESTS_SSL_CONFIG_FILTER_PARAM) $(SSLTESTS_IGNORE_PROTOCOLS_PARAM) $(SSLTESTS_IGNORE_CIPHERS_PARAM) $(SSLTESTS_USE_OPENSSL_CLIENT_PARAM) $(SSLTESTS_USE_GNUTLS_CLIENT_PARAM) $(SSLTESTS_USE_NSS_CLIENT_PARAM) $(SSLTESTS_SERVER_SHUTDOWN_OUTPUT_PARAM) $(SSLTESTS_WIRESHARK_CAPTURE_PARAM) \
